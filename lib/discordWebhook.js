...
const SUBMISSIONS_PING_ROLE   = process.env.DISCORD_SUBMISSIONS_PING_ROLE_ID || '';
...
export async function notifySubmission(data) {
    ...
    author: {
      name: discordUser?.displayName && discordUser?.username
        ...
        : discordUser?.username
          ? `@${discordUser.username}`
          : username,
      icon_url: discordUser?.id && discordUser?.avatar
        ? `https://cdn.discordapp.com/avatars/${discordUser.id}/${discordUser.avatar}.png?size=128`
        : `https://cdn.discordapp.com/embed/avatars/${discordUser?.id ? Number(BigInt(discordUser.id) >> 22n) % 6 : 0}.png`,
    },
    title: '🚩 Nueva Submission',
    fields,
    footer: {
      text: `Submission #${submissionId} · ${fmtDate(new Date())}`,
      icon_url: 'https://uy-demonlist.vercel.app/assets/logo.png',
    },
  };
  ...
  const payload = {
    ...
    content: SUBMISSIONS_PING_ROLE ? `<@&${SUBMISSIONS_PING_ROLE}>` : undefined,
    embeds: [embed],
  };
...
export async function notifyDecision(data) {
  ...
  const {
    decision, submissionId, levelName, playerName, playerDiscordId, staffName, youtubeLink,
    rejectionReason, approvalNote, isNewLevel, levelPosition, aredlPosition,
    victorNumber, totalVictors,
  } = data;

  const playerMention = playerDiscordId ? `<@${playerDiscordId}>` : null;
  const playerDisplay = playerMention
  ...
  const fields = [
    { name: '🎮 Nivel',   value: levelName,     inline: true },
    { name: '👤 Jugador', value: playerDisplay,  inline: true },
  ...
  const payload = {
    username: 'UY Demonlist',
    avatar_url: 'https://uy-demonlist.vercel.app/assets/logo.png',
    content: playerDiscordId ? `<@${playerDiscordId}>` : undefined,
    embeds: [embed],
  };
...
export async function notifySanction(data) {
  ...
  const {
    action, targetName, targetUsername, targetDiscordId, targetAvatar,
    staffName, staffUsername, staffDiscordId, staffAvatar,
  ...
  const targetAvatarUrl = targetDiscordId && targetAvatar
    ...
    : targetDiscordId
      ? `https://cdn.discordapp.com/embed/avatars/${Number(BigInt(targetDiscordId) >> 22n) % 6}.png`
      : 'https://cdn.discordapp.com/embed/avatars/0.png';
  ...
  const fields = [
    {
      ...
      value: targetUsername && targetUsername !== targetName
        ? `**${targetName}**\n@${targetUsername}`
        : `**${targetName}**`,
      inline: true,
    },
    {
      name: '🛡️ Staff',
      value: staffUsername && staffUsername !== staffName
        ? `**${staffName}**\n@${staffUsername}`
        : `**${staffName}**`,
      inline: true,
    },
  ...
  const embed = {
    ...
    thumbnail: { url: staffAvatarUrl },
    fields,
    footer: {
      text: `UY Demonlist · ${fmtDate(new Date())}`,
      icon_url: 'https://uy-demonlist.vercel.app/assets/logo.png',
    },
  };

...
export async function notifyDecision(data) {
  if (!DECISION_WEBHOOK_URL) return;

  const { decision, submissionId, levelName, playerName, staffName, youtubeLink, rejectionReason, approvalNote } = data;
  ...
  if (approved && approvalNote) {
    fields.push({ name: '💬 Nota del staff', value: approvalNote, inline: false });
  }
  ...
  const embed = {
    color,
    title: `${emoji} Submission #${submissionId} — ${label}`,
    fields,
    footer: { text: `Submission #${submissionId} · ${fmtDate(new Date())}` },
  };
  if (thumbUrl) embed.thumbnail = { url: thumbUrl };

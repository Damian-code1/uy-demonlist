...
async function getUserByDiscordId(discordId) {
  ...
  try {
    const [rows] = await query(
      'SELECT id, discord_id, discord_username, discord_display_name, discord_avatar, role FROM users WHERE discord_id = ? LIMIT 1',
      [discordId]
    );
    return rows[0] || null;
  } catch {
